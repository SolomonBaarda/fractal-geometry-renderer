...
#info Menger sponge and Mandelbulb
...
{ \
  return Union5 \
    ...
    , Diffuse(srand(PRNG, 4), Mandelbulb(tag, compose(Rotate(vec4(sin(pi/4.0) *normalize(vec3(1.0)), cos(pi/4.0))), Translate(vec3(0.0, 0.0, 2.0 + sqrt(8.0)/3.0))), V, 8, 10), V, 1.0) \
    ); \
}
