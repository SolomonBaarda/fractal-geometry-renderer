...
float signedDistanceEstimation(const Vector3& position)
{
	return spheres[8].signedDistanceEstimation(position);
}
...
/// <param name="maximumRaySteps"></param>
/// <param name="maximumDistance"></param>
/// <param name="distanceEstimator"></param>
/// <returns></returns>
float trace(Vector3 from, Vector3 direction, const uint32_t maximumRaySteps, const uint32_t maximumDistance)
{
	...
	for (steps = 0; steps < maximumRaySteps; steps++)
	{
		...
		float distance = signedDistanceEstimation(p);
...
int main(int argc, char* argv[])
{
	...
	const int32_t totalSamplesAllowed = 1;
...
#pragma omp parallel for schedule(dynamic, 1) private(r) // OpenMP
	...
	{
		fprintf(stderr, "\rRendering (%d spp) %5.2f%%", totalSamplesAllowed * 4, 100. * y / (static_cast<int64_t>(height) - 1));

		uint16_t Xi[3] = { 0, 0, static_cast<uint16_t>(y * y * y) };
		...
		for (int32_t x = 0; x < width; x++)
		{
			...
			// 2x2 subpixel rows
			for (int32_t sy = 0, i = (height - y - 1) * width + x; sy < 2; sy++)
			{
				// 2x2 subpixel cols
				for (int32_t sx = 0; sx < 2; sx++, r = Vector3())
				{
					...
					for (int32_t currentNumberOfSamples = 0; currentNumberOfSamples < totalSamplesAllowed; currentNumberOfSamples++)
					{
						...
						r = r + trace(cam.origin + d * 140, d.normalise(), 100, 1000);
						//r = r + radiance(Ray(cam.origin + d * 140, d.normalise()), 0, Xi) * (1. / totalSamplesAllowed);
					}
